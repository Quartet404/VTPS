...
async function fetchRandomUsers() {
    try {
        const response = await fetch('https://randomuser.me/api/?results=50'); // Запит 10 користувачів
...
function displayTeachers(teachers, page = 1) {
    ...
    updateTeacherPagination(teachers.length, page);
}
...
fetchAndFormatUsers().then(users => {
    teachers = users;
    displayTeachers(teachers, currentTeacherPage);
    displayStatistics(sortStatistics(teachers, currentSort.sortBy, currentSort.sortDirection), currentTablePage);
...
function displayStatistics(teachers, page = 1) {
    const tableBody = document.getElementById('table-body');
    tableBody.innerHTML = '';

    const startIndex = (page - 1) * tableEntriesPerPage;
    const endIndex = startIndex + tableEntriesPerPage;
    const paginatedTeachers = teachers.slice(startIndex, endIndex);
    ...
    updateTablePagination(teachers.length, page);
}

function updateTeacherPagination(totalTeachers, page) {
    const paginationContainer = document.getElementById('teacher-pagination');
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        pageButton.addEventListener('click', () => {
            currentTeacherPage = i;
            displayTeachers(teachers, i);
        });
        paginationContainer.appendChild(pageButton);
    }
}

function updateTablePagination(totalTeachers, page) {
    const paginationContainer = document.getElementById('table-pagination');
    paginationContainer.innerHTML = '';

    const totalPages = Math.ceil(totalTeachers / tableEntriesPerPage);
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        pageButton.addEventListener('click', () => {
            currentTablePage = i;
            displayStatistics(sortStatistics(filterAndSortTeachers(teachers), currentSort.sortBy, currentSort.sortDirection), i);
        });
        paginationContainer.appendChild(pageButton);
    }
}
...
document.querySelector('.nav-find').addEventListener('click', () => {
    const searchQuery = document.getElementById('nav-search').value;
    const searchResults = searchTeachers(teachers, searchQuery);
...
function searchTeachers(teachers, query) {
    ...
    return teachers.filter(teacher => {
        const age = parseInt(query, 10);
        return teacher.full_name.toLowerCase().includes(query) || 
               teacher.note.toLowerCase().includes(query) || 
               (age && teacher.age === age);
    });
}
...
document.getElementById('country-filter').addEventListener('change', () => {
    ...
    const filteredTeachers = filterAndSortTeachers(teachers);
    displayTeachers(filteredTeachers, currentTeacherPage);
});
...
document.getElementById('sort-name').addEventListener('click', () => {
    ...
    displayStatistics(sortedTeachers, currentTablePage);
});
